...
ml2_fslsdn_opts = [
    ...
    cfg.StrOpt('crd_auth_url', default='http://127.0.0.1:5000/v2.0/',
               help=_("CRD Auth URL")),
    cfg.StrOpt('crd_url',
...
class FslsdnMechanismDriver(api.MechanismDriver):
    ...
    def initialize(self):
        ...
        LOG.info("Initializing CRD client... ")
        crd_client_params = {
    ...
    def delete_network_postcommit(self, context):
        ...
        LOG.debug("delete_network update sent to CRD Server: %s",network['id'])
    ...
    def _prepare_crd_port(port):
        ...
        # Since CRD accepts one Fixed IP,handle only one fixed IP per port.
        if fixed_ips:
            crd_subnet_id = fixed_ips[0]['subnet_id']
            crd_ipaddress = fixed_ips[0]['ip_address']
        else:
    ...
    def _prepare_crd_subnet(subnet):
        ...
        crd_allocation_pools = ','.join(["%s-%s" % (p['start'], p['end'])
                                         for p in subnet['allocation_pools']])

        # Handling Host Routes
        crd_host_routes = ','.join(["%s-%s" % (r['destination'], r['nexthop'])
    ...
    def check_segment(self, segment):
        ...
        return network_type in [constants.TYPE_VLAN, constants.TYPE_VXLAN]
